...
const Score = ({ quiz }) => {
    ...
    return (
        <div className='bg-slate-900 z-[2] w-full rounded-lg py-5 flex flex-col gap-1 text-xl  bg-gradient-to-r from-blue-200 to-blue-300'>
            {
                loading ? (
                    <div className='text-center'>Loading...</div>
                ) : !loading && scores.length > 0 ? (
                    <div className=' border rounded-lg border-slate-600 overflow-hidden'>
                        <h3 className='px-3 text-2xl bg-slate-600 py-2 text-center text-white'>Results</h3>
                            ...
                            [...scores].reverse().map((score, index) => (
                                <div className='flex justify-between items-center py-3 border-t border-slate-600 px-5' key={index}>
                                    <span className='flex flex-col md:flex-row gap-1 items-center'>
                                        ...
                                        <p className='text-xs md:text-sm text-slate-300 text-black'>
